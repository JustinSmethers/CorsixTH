...
test("playable loop: build, hire, route, treat, save, and restore objective progress", async ({
  ...
  await expect(page.getByTestId("treat")).toBeEnabled();
  await expect(page.getByTestId("build-diagnosis-room")).toBeEnabled();
  await expect(page.getByTestId("hire-receptionist")).toBeEnabled();
  await expect(page.getByTestId("take-loan")).toBeEnabled();
  await expect(page.getByTestId("start-research")).toBeDisabled();
  await expect(page.getByTestId("admissions-toggle")).toBeEnabled();
  await expect(page.getByTestId("admission-policy")).toBeEnabled();
  await expect(page.getByTestId("pricing-policy")).toBeEnabled();
