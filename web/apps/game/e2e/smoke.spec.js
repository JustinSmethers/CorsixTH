...
test("phase 7 keyboard shortcuts save and load active slot", async ({ page }) => {
    ...
    await page.keyboard.press("F1");
    await expect(page.locator(":focus")).toHaveAttribute("data-testid", "loan-status");
    await expect(page.getByTestId("cash")).toHaveText(cashBeforeBankManagerFocus);
    await expect(page.getByTestId("loan-status")).toHaveText(loanStatusBeforeFocus);
    await page.getByTestId("playfield").focus();
    const cashBeforeBankStatsFocus = (await page.getByTestId("cash").textContent()) ?? "";
    const financeLedgerBeforeFocus = (await page.getByTestId("finance-ledger").textContent()) ?? "";
    await page.keyboard.press("F2");
    await expect(page.locator(":focus")).toHaveAttribute("data-testid", "finance-ledger");
    await expect(page.getByTestId("cash")).toHaveText(cashBeforeBankStatsFocus);
    await expect(page.getByTestId("finance-ledger")).toHaveText(financeLedgerBeforeFocus);
    await page.getByTestId("playfield").focus();
