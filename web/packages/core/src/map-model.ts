...
export interface TileMapOptions {
  ...
  tileDefinitions: ReadonlyArray<TileDefinition>;
}
...
function assertValidTileDefinitions(tileDefinitions: ReadonlyArray<TileDefinition>): Map<number, TileDefinition> {
  if (tileDefinitions.length === 0) {
    throw new Error("Tile definitions must not be empty");
  }

  const byId = new Map<number, TileDefinition>();
...
export class TileMap {
  ...
  private readonly tileIds: Int32Array;
  private readonly definitionsById: Map<number, TileDefinition>;
  ...
  static fromRows(rows: ReadonlyArray<ReadonlyArray<number>>, tileDefinitions: ReadonlyArray<TileDefinition>): TileMap {
    if (rows.length === 0) {
  ...
  toIndex(position: GridPosition): number {
  ...
  toPosition(index: number): GridPosition {
  ...
  tileIdAt(position: GridPosition): number {
    const index = this.toIndex(position);
    return this.tileIds[index] ?? 0;
  }

  metadataAt(position: GridPosition): TileMetadata {
  ...
  isPassable(position: GridPosition): boolean {
    return this.metadataAt(position).passable;
  }

  movementCostAt(position: GridPosition): number {
    return this.metadataAt(position).movementCost;
  }

  clonePosition(position: GridPosition): GridPosition {
