...
import type { GridPosition } from "../src/command-contract";
...
function createBenchMap(width: number, height: number, seed: number): TileMap {
...
function createQueries(width: number, height: number, count: number, seed: number): Array<{ start: GridPosition; goal: GridPosition }> {
  const rng = new DeterministicRng(seed);
  const queries: Array<{ start: GridPosition; goal: GridPosition }> = [];
...
function measureAverageSolveTimeMs(service: DeterministicPathfindingService, queries: Array<{ start: GridPosition; goal: GridPosition }>): number {
