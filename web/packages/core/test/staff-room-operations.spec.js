...
describe("phase 7 slice 2 staff lifecycle and room operations", () => {
    ...
    it("rejects staff placement on objects, staff, and patients", () => {
        ...
        const afterHire = simulation.getState();
        expect(afterHire.entities.staff).toHaveLength(beforeHire.entities.staff.length);
        expect(afterHire.cash).toBe(beforeHire.cash);
    });
    it("rejects staff moves to occupied target tiles", () => {
    ...
    it("requires surgeon-qualified doctors for Operating Theatre treatment", () => {
        ...
        blocked.execute({ type: "open-room", roomType: "operating-theatre", position: { x: 1, y: 6 } });
        oneSurgeon.execute({ type: "open-room", roomType: "operating-theatre", position: { x: 1, y: 6 } });
        specialized.execute({ type: "open-room", roomType: "operating-theatre", position: { x: 1, y: 6 } });
        ...
        for (let tick = 1; tick <= 16; tick += 1) {
            ...
            specialized.execute({ type: "tick", count: 1 });
            const specialistRoom = specialized.getState().entities.rooms.find((room) => room.roomType === "operating-theatre");
        ...
        expect(blocked.getState().entities.rooms.find((room) => room.roomType === "operating-theatre")).toMatchObject({
    ...
    it("requires an open staffed ward before surgery patients enter Operating Theatre", () => {
        ...
        missingWard.execute({ type: "open-room", roomType: "operating-theatre", position: { x: 1, y: 8 } });
        ready.execute({ type: "open-room", roomType: "operating-theatre", position: { x: 1, y: 8 } });
    ...
    it("requires psychiatrist-qualified doctors for psychiatry treatment", () => {
        ...
        blocked.execute({ type: "open-room", roomType: "psychiatry", position: { x: 1, y: 8 } });
        specialized.execute({ type: "open-room", roomType: "psychiatry", position: { x: 1, y: 8 } });
        blocked.execute({ type: "hire-staff", role: "diagnostician", position: { x: 10, y: 4 } });
        specialized.execute({ type: "hire-staff", role: "diagnostician", initialSpecialties: ["psychiatrist"], position: { x: 10, y: 4 } });
        blocked.execute({ type: "admit-patient", severity: 2, diseaseId: "king-complex", position: { x: 2, y: 4 } });
        specialized.execute({ type: "admit-patient", severity: 2, diseaseId: "king-complex", position: { x: 2, y: 4 } });
    ...
    it("requires disease-specific treatment rooms for pharmacy, inflation, slack tongue, fracture, hair, jelly, decontamination, electrolysis, and specialist diseases", () => {
        ...
        specialized.execute({ type: "admit-patient", severity: 2, diseaseId: "sleepy-bones", position: { x: 2, y: 4 } });
        specialized.execute({ type: "tick", count: 4 });
        expect(specialized.getState().entities.waitingPatients[0]).toMatchObject({
            diseaseId: "sleepy-bones",
        ...
        fallback.execute({ type: "admit-patient", severity: 2, diseaseId: "sleepy-bones", position: { x: 2, y: 4 } });
        ...
        const inflation = new DeterministicSimulation(72130, { bounds: { width: 12, height: 12 } });
        inflation.execute({ type: "open-room", roomType: "inflation-room", position: { x: 1, y: 7 } });
        inflation.execute({ type: "hire-staff", role: "diagnostician", position: { x: 8, y: 4 } });
        const inflationRoom = inflation.getState().entities.rooms.find((room) => room.roomType === "inflation-room");
        expect(inflationRoom).toBeTruthy();
        inflation.execute({ type: "admit-patient", severity: 3, diseaseId: "cranial-pressure", position: { x: 2, y: 4 } });
        inflation.execute({ type: "tick", count: 8 });
        expect(inflation.getState().entities.waitingPatients[0]).toMatchObject({
            diseaseId: "cranial-pressure",
            preferredTreatmentRoomType: "inflation-room",
            status: "walking-to-treatment",
            assignedRoomId: inflationRoom.id
        });
        const missingInflation = new DeterministicSimulation(72133, { bounds: { width: 12, height: 12 } });
        missingInflation.execute({ type: "admit-patient", severity: 3, diseaseId: "cranial-pressure", position: { x: 2, y: 4 } });
        missingInflation.execute({ type: "tick", count: 5 });
        expect(missingInflation.getState().entities.waitingPatients[0]).toMatchObject({
            diseaseId: "cranial-pressure",
            preferredTreatmentRoomType: "inflation-room",
            status: "awaiting-treatment"
        });
        expect(missingInflation.getState().entities.waitingPatients[0]?.assignedRoomId).toBeNull();
        const slack = new DeterministicSimulation(72134, { bounds: { width: 12, height: 12 } });
        slack.execute({ type: "open-room", roomType: "slack-tongue-clinic", position: { x: 1, y: 7 } });
        slack.execute({ type: "hire-staff", role: "diagnostician", position: { x: 8, y: 4 } });
        const slackTongueClinic = slack.getState().entities.rooms.find((room) => room.roomType === "slack-tongue-clinic");
        expect(slackTongueClinic).toBeTruthy();
        slack.execute({ type: "admit-patient", severity: 2, diseaseId: "slack-tongue", position: { x: 2, y: 4 } });
        slack.execute({ type: "tick", count: 4 });
        expect(slack.getState().entities.waitingPatients[0]).toMatchObject({
            diseaseId: "slack-tongue",
            preferredTreatmentRoomType: "slack-tongue-clinic",
            status: "walking-to-treatment",
            assignedRoomId: slackTongueClinic.id
        });
        const missingSlack = new DeterministicSimulation(72135, { bounds: { width: 12, height: 12 } });
        missingSlack.execute({ type: "admit-patient", severity: 2, diseaseId: "slack-tongue", position: { x: 2, y: 4 } });
        missingSlack.execute({ type: "tick", count: 5 });
        expect(missingSlack.getState().entities.waitingPatients[0]).toMatchObject({
            diseaseId: "slack-tongue",
            preferredTreatmentRoomType: "slack-tongue-clinic",
            status: "awaiting-treatment"
        });
        expect(missingSlack.getState().entities.waitingPatients[0]?.assignedRoomId).toBeNull();
    ...
    it("does not let unstaffed Operating Theatre cases block staffed general treatment rooms", () => {
        ...
        const specialistPatient = simulation.getState().entities.waitingPatients.find((patient) => patient.diseaseId === "spare-ribs");
