...
import { runVisualSnapshotFixture } from "../src/index";
...
describe("renderer visual snapshots", () => {
    ...
    it("fails when baseline drift exceeds max pixel-diff threshold", () => {
        ...
        const broken = {
            ...parsed,
            scenarios: parsed.scenarios.map((scenario, index) => index !== 0
                ? scenario
                : {
                    ...scenario,
                    baselineRows: scenario.baselineRows.map((row, rowIndex) => rowIndex === 0 ? `w${row.slice(1)}` : row)
                })
        };
