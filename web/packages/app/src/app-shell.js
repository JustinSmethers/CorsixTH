...
const ROOM_TYPE_COLORS = {
    ...
    research: "#70a6b8",
    toilets: "#8eb28f",
    "training-room": "#6fa8dc",
    "inflation-room": "#e0bd4f",
    "slack-tongue-clinic": "#e07a8a",
    "fracture-clinic": "#d0a15d",
    "hair-restoration": "#76b7a3",
    "jelly-vat": "#8ebf6a",
    decontamination: "#67b8b2",
    electrolysis: "#83a1dc",
    "dna-fixer": "#c59bf7"
};
const DIAGNOSIS_RESEARCH_ROOM_TYPES = new Set(["diagnosis", "cardiogram", "scanner", "ultrascan", "blood-machine", "x-ray", "general-diagnosis"]);
...
export function formatOriginalUiStripControlLabel(control) {
    const labels = {
        ...
        "build-staff-room": "Build Staff Room",
        "build-research-room": "Build Research",
        "build-toilets": "Build Toilets",
        "build-training-room": "Build Training",
        "build-inflation-room": "Build Inflation",
        "build-slack-tongue-clinic": "Build Tongue",
        "build-fracture-clinic-room": "Build Fracture",
        "build-hair-restoration-room": "Build Hair",
        "build-jelly-vat-room": "Build Jelly",
        "build-decontamination-room": "Build Decon",
        "build-electrolysis-room": "Build Electro",
...
function roomTypeDisplayName(roomType, languageSummary) {
    ...
    if (roomType === "psychiatry") {
        return languageSummary?.roomNames?.[roomType] ?? "Psychiatry";
    }
    if (roomType === "training-room") {
        return languageSummary?.roomNames?.[roomType] ?? "Training Room";
    }
    if (roomType === "staff-room") {
        return languageSummary?.roomNames?.[roomType] ?? "Staff Room";
    }
    if (roomType === "research") {
        return languageSummary?.roomNames?.[roomType] ?? "Research";
    }
    if (roomType === "toilets") {
        return languageSummary?.roomNames?.[roomType] ?? "Toilets";
    }
    if (roomType === "slack-tongue-clinic") {
        return languageSummary?.roomNames?.[roomType] ?? "Slack Tongue Clinic";
    }
    if (roomType === "hair-restoration") {
        return languageSummary?.roomNames?.[roomType] ?? "Hair Restoration";
    }
    if (roomType === "jelly-vat") {
        return languageSummary?.roomNames?.[roomType] ?? "Jelly Vat";
    }
    if (roomType === "decontamination") {
        return languageSummary?.roomNames?.[roomType] ?? "Decontamination";
    }
    if (roomType === "electrolysis") {
        return languageSummary?.roomNames?.[roomType] ?? "Electrolysis";
    }
...
export function formatSpecializedTreatmentRoomsStatus(telemetry) {
    return `Specialized rooms: pharmacy ${telemetry.openPharmacyRooms}, Operating Theatre ${telemetry.openOperatingTheatreRooms ?? 0}, specialist ${telemetry.openSpecialistRooms}, Psychiatry ${telemetry.openPsychiatryRooms ?? 0}, Training Room ${telemetry.openTrainingRooms ?? 0}, Inflation Room ${telemetry.openInflationRooms ?? 0}, Slack Tongue Clinic ${telemetry.openSlackTongueClinicRooms ?? 0}, Fracture Clinic ${telemetry.openFractureClinicRooms ?? 0}, Hair Restoration ${telemetry.openHairRestorationRooms ?? 0}, Jelly Vat ${telemetry.openJellyVatRooms ?? 0}, Decontamination ${telemetry.openDecontaminationRooms ?? 0}, Electrolysis ${telemetry.openElectrolysisRooms ?? 0}, DNA Fixer ${telemetry.openDnaFixerRooms ?? 0}`;
}
...
const ORIGINAL_UI_STRIP_CONTROLS = [
    ...
    { id: "build-staff-room", label: formatOriginalUiStripControlLabel("build-staff-room") },
    { id: "build-research-room", label: formatOriginalUiStripControlLabel("build-research-room") },
    { id: "build-toilets", label: formatOriginalUiStripControlLabel("build-toilets") },
    { id: "build-training-room", label: formatOriginalUiStripControlLabel("build-training-room") },
    { id: "build-inflation-room", label: formatOriginalUiStripControlLabel("build-inflation-room") },
    { id: "build-slack-tongue-clinic", label: formatOriginalUiStripControlLabel("build-slack-tongue-clinic") },
    ...
    { id: "build-jelly-vat-room", label: formatOriginalUiStripControlLabel("build-jelly-vat-room") },
...
function scenarioObjectAvailabilityDetails(scenario, languageSummary, telemetry = {}) {
    ...
    for (const object of orderedObjects) {
        ...
        if (hasTelemetryBuckets && disabledIndices?.has(object.index)) {
            ...
            DIAGNOSIS_RESEARCH_ROOM_TYPES.has(object.roomType) &&
...
export function createRoomAvailabilityScheduleFromScenario(scenario) {
    ...
    return objectAvailability
        ...
        ...(DIAGNOSIS_RESEARCH_ROOM_TYPES.has(object.roomType) && object.startAvailable !== true && diagnosisResearchRequired !== null
...
export function mountAppShell(options) {
    ...
    >
      <header style="display:flex; align-items:flex-start; justify-content:space-between; gap:16px; margin-bottom:12px;">
        ...
        <div data-testid="controls" style="display:flex; flex-wrap:wrap; gap:8px; justify-content:flex-end;">
          ...
          <button type="button" data-testid="build-staff-room">${formatBuildRoomButtonLabel("staff-room")}</button>
          <button type="button" data-testid="build-research-room">${formatBuildRoomButtonLabel("research")}</button>
          <button type="button" data-testid="build-toilets">${formatBuildRoomButtonLabel("toilets")}</button>
          <button type="button" data-testid="build-training-room">${formatBuildRoomButtonLabel("training-room")}</button>
          <button type="button" data-testid="build-inflation-room">${formatBuildRoomButtonLabel("inflation-room")}</button>
          <button type="button" data-testid="build-slack-tongue-clinic">${formatBuildRoomButtonLabel("slack-tongue-clinic")}</button>
          ...
          <button type="button" data-testid="build-jelly-vat-room">${formatBuildRoomButtonLabel("jelly-vat")}</button>
  ...
  `;
    ...
    const buildStaffRoomButton = requiredElement(options.root, "[data-testid='build-staff-room']");
    const buildResearchRoomButton = requiredElement(options.root, "[data-testid='build-research-room']");
    const buildToiletsButton = requiredElement(options.root, "[data-testid='build-toilets']");
    const buildTrainingRoomButton = requiredElement(options.root, "[data-testid='build-training-room']");
    const buildInflationRoomButton = requiredElement(options.root, "[data-testid='build-inflation-room']");
    const buildSlackTongueClinicButton = requiredElement(options.root, "[data-testid='build-slack-tongue-clinic']");
    ...
    const buildJellyVatRoomButton = requiredElement(options.root, "[data-testid='build-jelly-vat-room']");
    ...
    const renderSelectionControls = () => {
        ...
        editRoomPanelToggleButton.disabled = !canToggleTreatmentRoomFromState(state, resolved?.type === "room" ? resolved.value : null, telemetry);
        ...
        telemetryElements.staffBreakToggleButton.disabled = !canToggleStaffBreakFromState(state, resolved?.type === "staff" ? resolved.value : null, telemetry);
        telemetryElements.treatmentRoomToggleButton.disabled = !canToggleTreatmentRoomFromState(state, resolved?.type === "room" ? resolved.value : null, telemetry);
    };
    const renderRuntime = () => {
        const currentMap = campaignMapSummaryAt(hospitalView, campaignLevelIndex(hospitalView));
        ...
        buildStaffRoomButton.textContent = formatBuildRoomButtonLabel("staff-room", telemetry, hospitalView?.languageSummary ?? null);
        buildResearchRoomButton.textContent = formatBuildRoomButtonLabel("research", telemetry, hospitalView?.languageSummary ?? null);
        buildToiletsButton.textContent = formatBuildRoomButtonLabel("toilets", telemetry, hospitalView?.languageSummary ?? null);
        buildTrainingRoomButton.textContent = formatBuildRoomButtonLabel("training-room", telemetry, hospitalView?.languageSummary ?? null);
        ...
        buildSlackTongueClinicButton.textContent = formatBuildRoomButtonLabel("slack-tongue-clinic", telemetry, hospitalView?.languageSummary ?? null);
        ...
        buildJellyVatRoomButton.textContent = formatBuildRoomButtonLabel("jelly-vat", telemetry, hospitalView?.languageSummary ?? null);
        ...
        buildStaffRoomButton.disabled = !canBuildRoomFromTelemetry("staff-room", telemetry);
        buildResearchRoomButton.disabled = !canBuildRoomFromTelemetry("research", telemetry);
        buildToiletsButton.disabled = !canBuildRoomFromTelemetry("toilets", telemetry);
        buildTrainingRoomButton.disabled = !canBuildRoomFromTelemetry("training-room", telemetry);
        buildInflationRoomButton.disabled = !canBuildRoomFromTelemetry("inflation-room", telemetry);
        buildSlackTongueClinicButton.disabled = !canBuildRoomFromTelemetry("slack-tongue-clinic", telemetry);
        ...
        buildJellyVatRoomButton.disabled = !canBuildRoomFromTelemetry("jelly-vat", telemetry);
    ...
    const onRestSelectedStaff = () => {
        const telemetry = orchestrator.telemetry();
        const resolved = selectedEntityFromState(orchestrator.getState(), selectedEntity);
        if (resolved?.type !== "staff" || !canRestStaffFromTelemetry(resolved.value, telemetry)) {
        ...
        const events = dispatchAndRender(orchestrator, telemetryElements, audioMixer, {
            ...
            action: "rest-staff",
            staffId: resolved.value.id,
            restType: bestStaffRestType(telemetry),
    ...
    const onTrainSelectedStaff = () => {
        const telemetry = orchestrator.telemetry();
        const resolved = selectedEntityFromState(orchestrator.getState(), selectedEntity);
        if (resolved?.type !== "staff" || !canTrainStaffFromTelemetry(resolved.value, telemetry)) {
    ...
    const onFireSelectedStaff = () => {
        const telemetry = orchestrator.telemetry();
        const resolved = selectedEntityFromState(orchestrator.getState(), selectedEntity);
        if (resolved?.type !== "staff" || !canFireStaff(resolved.value, telemetry)) {
    ...
    const onSellSelectedRoom = () => {
        const telemetry = orchestrator.telemetry();
        const resolved = selectedEntityFromState(orchestrator.getState(), selectedEntity);
        if (resolved?.type !== "room" || !canSellRoom(resolved.value, telemetry)) {
    ...
    const onRepairSelectedRoom = () => {
        ...
        if (!canRepairRoomFromTelemetry(resolved.value, telemetry)) {
            actionStatus.textContent = formatActionStatus("room.repair-blocked");
            renderRuntime();
            return;
        }
    ...
    const onStaffBreakToggle = () => {
        ...
        if (!canToggleStaffBreakFromState(state, staff, telemetry)) {
    ...
    const onTreatmentRoomToggle = () => {
        ...
        if (!canToggleTreatmentRoomFromState(state, room, telemetry)) {
    ...
    const onBuildWardRoom = () => {
        if (!canBuildRoomFromTelemetry("ward", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            roomType: "ward",
            orientation: "north",
            source: "ui:build-ward-room",
            label: `build ${roomTypeDisplayName("ward", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildPsychiatryRoom = () => {
        if (!canBuildRoomFromTelemetry("psychiatry", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            label: `build ${roomTypeDisplayName("psychiatry", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildTrainingRoom = () => {
        if (!canBuildRoomFromTelemetry("training-room", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            orientation: "north",
            source: "ui:build-training-room",
            label: `build ${roomTypeDisplayName("training-room", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildStaffRoom = () => {
        if (!canBuildRoomFromTelemetry("staff-room", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            roomType: "staff-room",
            orientation: "north",
            source: "ui:build-staff-room",
            label: `build ${roomTypeDisplayName("staff-room", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildResearchRoom = () => {
        if (!canBuildRoomFromTelemetry("research", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            label: `build ${roomTypeDisplayName("research", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildToilets = () => {
        if (!canBuildRoomFromTelemetry("toilets", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            roomType: "toilets",
            orientation: "north",
            source: "ui:build-toilets",
            label: `build ${roomTypeDisplayName("toilets", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildSlackTongueClinic = () => {
        if (!canBuildRoomFromTelemetry("slack-tongue-clinic", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            action: "build-room",
            roomType: "slack-tongue-clinic",
            orientation: "north",
            source: "ui:build-slack-tongue-clinic",
            label: `build ${roomTypeDisplayName("slack-tongue-clinic", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildFractureClinicRoom = () => {
        if (!canBuildRoomFromTelemetry("fracture-clinic", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            label: `build ${roomTypeDisplayName("fracture-clinic", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildHairRestorationRoom = () => {
        if (!canBuildRoomFromTelemetry("hair-restoration", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            action: "build-room",
            roomType: "hair-restoration",
            orientation: "north",
            source: "ui:build-hair-restoration-room",
            label: `build ${roomTypeDisplayName("hair-restoration", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildJellyVatRoom = () => {
        if (!canBuildRoomFromTelemetry("jelly-vat", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            roomType: "jelly-vat",
            orientation: "north",
            source: "ui:build-jelly-vat-room",
            label: `build ${roomTypeDisplayName("jelly-vat", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildDecontaminationRoom = () => {
        if (!canBuildRoomFromTelemetry("decontamination", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            orientation: "north",
            source: "ui:build-decontamination-room",
            label: `build ${roomTypeDisplayName("decontamination", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const onBuildElectrolysisRoom = () => {
        if (!canBuildRoomFromTelemetry("electrolysis", orchestrator.telemetry())) {
            actionStatus.textContent = formatActionStatus("room.build-blocked");
            renderRuntime();
            return;
        }
        placementAction = {
            ...
            orientation: "north",
            source: "ui:build-electrolysis-room",
            label: `build ${roomTypeDisplayName("electrolysis", hospitalView?.languageSummary ?? null)}`
        };
        placementPreview = null;
        selectedEntity = null;
        actionStatus.textContent = formatChoosePlacementActionStatus();
    ...
    const originalUiControlHandlers = new Map([
        ...
        ["build-training-room", onBuildTrainingRoom],
        ["build-staff-room", onBuildStaffRoom],
        ["build-research-room", onBuildResearchRoom],
        ["build-toilets", onBuildToilets],
        ["build-inflation-room", onBuildInflationRoom],
        ["build-slack-tongue-clinic", onBuildSlackTongueClinic],
        ["build-fracture-clinic-room", onBuildFractureClinicRoom],
        ["build-hair-restoration-room", onBuildHairRestorationRoom],
        ["build-jelly-vat-room", onBuildJellyVatRoom],
    ...
    buildStaffRoomButton.addEventListener("click", onBuildStaffRoom);
    buildResearchRoomButton.addEventListener("click", onBuildResearchRoom);
    buildToiletsButton.addEventListener("click", onBuildToilets);
    buildTrainingRoomButton.addEventListener("click", onBuildTrainingRoom);
    buildInflationRoomButton.addEventListener("click", onBuildInflationRoom);
    buildSlackTongueClinicButton.addEventListener("click", onBuildSlackTongueClinic);
    ...
    buildJellyVatRoomButton.addEventListener("click", onBuildJellyVatRoom);
    buildDecontaminationRoomButton.addEventListener("click", onBuildDecontaminationRoom);
    buildElectrolysisRoomButton.addEventListener("click", onBuildElectrolysisRoom);
    ...
    return {
        ...
        dispose: () => {
            ...
            buildStaffRoomButton.removeEventListener("click", onBuildStaffRoom);
            buildResearchRoomButton.removeEventListener("click", onBuildResearchRoom);
            buildToiletsButton.removeEventListener("click", onBuildToilets);
            buildTrainingRoomButton.removeEventListener("click", onBuildTrainingRoom);
            buildInflationRoomButton.removeEventListener("click", onBuildInflationRoom);
            buildSlackTongueClinicButton.removeEventListener("click", onBuildSlackTongueClinic);
            ...
            buildJellyVatRoomButton.removeEventListener("click", onBuildJellyVatRoom);
            buildDecontaminationRoomButton.removeEventListener("click", onBuildDecontaminationRoom);
            buildElectrolysisRoomButton.removeEventListener("click", onBuildElectrolysisRoom);
