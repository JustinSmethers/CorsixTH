...
import {
  parsePathfindingFixtureV0,
  runPathfindingFixture,
  type PathfindingFixtureScenarioResult
} from "../src/pathfinding-fixture";
...
describe("pathfinding fixture harness v0", () => {
  ...
  it("validates all deterministic golden routes", () => {
    ...
    const failed = result.scenarioResults.filter((scenario: PathfindingFixtureScenarioResult) => !scenario.passed);
  ...
  it("detects parity drift in scenario expectations", () => {
    ...
    expect(result.scenarioResults.some((scenario: PathfindingFixtureScenarioResult) => !scenario.passed)).toBe(true);
  });
});
